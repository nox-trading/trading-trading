...
struct ConCommon
  {
   ...
   __time32_t            overnight_prev_time;   // time of previous overnight
   //--- month reports
   __time32_t            overmonth_last_month;  // time of last report
   //--- performance base
...
struct ConSymbol
  {
   ...
   __time32_t            value_date;                  // value date for this security
...
struct LogRequest
  {
   ...
   __time32_t            from;                       // from time
   __time32_t            to;                         // to time
...
struct UserInfo
  {
   ...
   int               enable_change_password;     // allow to change password
   int               enable_read_only;           // allow to open/positions (TRUE-may not trade)
   int               enable_flags;               // additional flags
   //--- trade parameters
   int               leverage;                   // leverage
   int               agent_account;              // agent account
   //--- trade data
...
struct TradeTransInfo
  {
   ...
   __time32_t            expiration;                 // expiration time (server time in UNIX format)
   int               crc;                        // CRC
  };
#pragma pack(pop)
...
struct DailyReport
  {
   int               login;                      // login
   __time32_t            ctm;                        // daily report time
...
struct CServerInterface
  {
   ...
   //---            common functions
   virtual __time32_t      __stdcall TradeTime(void);
   ...
   virtual int         __stdcall ClientsChangeCredit(const int login,const ConGroup *grp,const double value,const __time32_t date,LPCSTR comment);
   ...
   virtual TradeRecord*__stdcall OrdersGet(const __time32_t from,const __time32_t to,const int *logins,const int count,int* total);
   virtual TradeRecord*__stdcall OrdersGetOpen(const UserInfo* user,int* total);
   virtual TradeRecord*__stdcall OrdersGetClosed(const __time32_t from,const __time32_t to,const int *logins,const int count,int* total);
   //--- trade info access
   ...
   virtual int         __stdcall HistoryPrices(LPCSTR symbol,double *prices,__time32_t *ctm,int *dir);
   ...
   virtual DailyReport* __stdcall DailyGet(LPCSTR group,const __time32_t from,const __time32_t to,int* logins,const int logins_total,int *daily_total);
   ...
   //--- special checks
   virtual int          __stdcall TradesCheckSessions(const ConSymbol *symbol,const __time32_t ctm);
   ...
   virtual TickAPI*    __stdcall  HistoryTicksGet(LPCSTR symbol,const __time32_t from,const __time32_t to,const char ticks_flags,int* total);
   //--- request server logs
...
void APIENTRY        MtSrvScheduler(const __time32_t curtime);
...
void APIENTRY        MtSrvStatementsRollovers(const int month_state,const __time32_t ctm,const tm *ttm);
//+------------------------------------------------------------------+
