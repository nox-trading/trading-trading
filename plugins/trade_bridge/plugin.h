...
namespace mt4
{
	...
	public:
		...
		void publish_chart(time_t from_time);
	...
	private:
		...
		void on_trade_request(int id);

		void publish_chart_by_symbol(const ConSymbol& symbol, int period, time_t from_time);

		void publish_group_symbols(const ConGroup& group);
		void publish_symbol_for_groups(const ConSymbol& symbol);

		bool should_stop_task(const std::string_view task_name, int iteration);
		...
		std::string						m_topic_name_feed_tick;
		std::string						m_topic_name_con_symbol;
		std::string						m_topic_name_mt4_candle;
	};
}